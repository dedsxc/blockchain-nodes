...
if [ ! -d "$HOME/data" ] || [ -z "$(ls -A $HOME/data)" ]; then
    ...
    wget -O $HOME/config/genesis.json https://raw.githubusercontent.com/maticnetwork/launch/master/mainnet-v1/sentry/sentry/heimdall/config/genesis.json
    ...
    curl -sL https://snapshot-download.polygon.technology/snapdown.sh | sed '/sudo apt-get update -y/d; /sudo apt-get install -y zstd pv aria2/d' | bash -s -- --network mainnet --client heimdall --extract-dir $HOME/data --validate-checksum true > $HOME/snapshot.log
fi
